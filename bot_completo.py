import os
import re
import requests
import asyncio
from threading import Thread
from flask import Flask
from telegram import Update
from telegram.ext import ApplicationBuilder, CommandHandler, ConversationHandler, MessageHandler, ContextTypes, filters

# --- SERVIDOR WEB PARA RENDER ---
app_flask = Flask(__name__)
@app_flask.route('/')
def health(): return "Bot en linea", 200

def run_flask():
    port = int(os.environ.get("PORT", 8080))
    app_flask.run(host='0.0.0.0', port=port)

# --- CONFIGURACIÓN ---
TOKEN = os.environ.get("TOKEN", "8634912458:AAHVJBE8vXTP9aLcfSQ3RbfcRRf2_qXVQl8")
ORIGEN, DESTINO, FECHA, HORA_INI, HORA_FIN = range(5)
busquedas_activas = {}

def buscar_vuelos(b):
    vuelos_validos = []
    url = "https://google.com"
    parametros = {
        "q": f"vuelos binter {b['origen']} a {b['destino']} {b['fecha']}",
        "hl": "es", "gl": "es"
    }
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"}
    try:
        response = requests.get(url, params=parametros, headers=headers, timeout=15)
        texto = response.text
        # Captura formatos como 15:30, 15.30 o 15-30
        patrones = re.findall(r'\b(?:[0-1]?[0-9]|2[0-3])[:\.\-][0-5][0-9]\b', texto)
        horas = [h.replace('.', ':').replace('-', ':').zfill(5) for h in patrones]
        if any(x in texto for x in ["Binter", "Canarias", "NT"]):
            for h in set(horas):
                if b['hora_ini'] <= h <= b['hora_fin']:
                    vuelos_validos.append(h)
    except Exception as e:
        print(f"Error en busqueda: {e}")
    return sorted(list(set(vuelos_validos)))

async def monitor(context: ContextTypes.DEFAULT_TYPE):
    chat_id = context.job.chat_id
    if chat_id not in busquedas_activas: return
    b = busquedas_activas[chat_id]
    vuelos = await asyncio.to_thread(buscar_vuelos, b)
    if vuelos:
        msg = f"✈️ ¡VUELOS ENCONTRADOS!\n{b['origen']}➔{b['destino']} ({b['fecha']})\n✅ {', '.join(vuelos)}"
        await context.bot.send_message(chat_id=chat_id, text=msg)
        context.job.schedule_removal()
        del busquedas_activas[chat_id]

# --- HANDLERS ---
async def start(u, c): await u.message.reply_text("Monitor Cloud Activo. /buscar")
async def buscar(u, c): await u.message.reply_text("Origen (LPA):"); return ORIGEN
async def origen(u, c): c.user_data["origen"] = u.message.text.upper(); await u.message.reply_text("Destino (TFN):"); return DESTINO
async def destino(u, c): c.user_data["destino"] = u.message.text.upper(); await u.message.reply_text("Fecha (AAAA-MM-DD):"); return FECHA
async def fecha(u, c): c.user_data["fecha"] = u.message.text; await u.message.reply_text("Hora inicio (17:00):"); return HORA_INI
async def hora_ini(u, c): c.user_data["hora_ini"] = u.message.text; await u.message.reply_text("Hora fin (22:00):"); return HORA_FIN
async def hora_fin(u, c):
    c.user_data["hora_fin"] = u.message.text
    chat_id = u.effective_chat.id
    busquedas_activas[chat_id] = {
        "origen": c.user_data["origen"], "destino": c.user_data["destino"],
        "fecha": c.user_data["fecha"], "hora_ini": c.user_data["hora_ini"].zfill(5),
        "hora_fin": c.user_data["hora_fin"].zfill(5)
    }
    c.job_queue.run_repeating(monitor, interval=180, first=5, chat_id=chat_id)
    await u.message.reply_text("✅ Buscando en la nube cada 3 min...")
    return ConversationHandler.END

if __name__ == "__main__":
    Thread(target=run_flask).start()
    app = ApplicationBuilder().token(TOKEN).build()
    conv = ConversationHandler(
        entry_points=[CommandHandler("buscar", buscar)],
        states={ORIGEN:[MessageHandler(filters.TEXT,origen)],DESTINO:[MessageHandler(filters.TEXT,destino)],FECHA:[MessageHandler(filters.TEXT,fecha)],HORA_INI:[MessageHandler(filters.TEXT,hora_ini)],HORA_FIN:[MessageHandler(filters.TEXT,hora_fin)]},
        fallbacks=[]
    )
    app.add_handler(CommandHandler("start", start))
    app.add_handler(conv)
    app.run_polling()
